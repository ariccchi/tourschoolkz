...
const navigate = useNavigate();
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch('http://localhost:8888/tourschoolphp/fintestlesson.php', {
            method: 'POST',
            headers: {
    ...
    const sendTestData = async () => {
      try {
        const response = await fetch('http://localhost:8888/tourschoolphp/fintestresult.php', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
