...
function SummaryPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response1 = await axios.post('http://localhost:8888/tourschoolphp/resultoflesson.php', {
      ...
      const response3 = await axios.post('http://localhost:8888/tourschoolphp/checkfulltest.php', {
        title: titleWithSpaces,
        user_id: user_id,
      ...
      const response2 = await axios.post('http://localhost:8888/tourschoolphp/resultoffinaltestlesson.php', {
        title: titleWithSpaces,
        user_id: user_id,
...
const countTriesTwo = responseDataArray.filter(item => item.tries === 2).length;
  ...
  const handleSaveResults = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8888/tourschoolphp/saveResults.php', {
        user_id: user_id,
        lesson_id: titleWithSpaces,
        is_completed: true,
        result: fulltestandlesson,
